  ...
  const table = useReactTable({
    ...
    columns: [
      ...columns,
      {
        ...
        id: 'actions',
        cell: ({ row }) => {
          const data = row.original as TProcurement

          const isAuthorized =
            profile?.data.role === 'ADMIN' ||
            profile?.data.role === 'SUPERADMIN' ||
            data.requestor === profile?.data._id

          return (
            <div className='flex items-center justify-center gap-1'>
              {isAuthorized ? (
                <>
                  <UpdateProcurement id={data._id} />
                  <DeleteProcurement id={data._id} />
                  <ViewProcurement id={data._id} />
                </>
              ) : (
